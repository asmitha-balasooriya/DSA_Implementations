...
def complete_BST_seq(A, B):
    ...
    B = complete_BST_seq(A[:root_index], B)

    # Recursive call for the right half
    B = complete_BST_seq(A[root_index+1:], B)

    return B

# Function to build the complete BST
def build_complete_BST(A):
    A.sort()  # Sort the input array first
    B = []
    B = complete_BST_seq(A, B)  # Generate the re-arranged sequence for a complete BST
...
def main():
    ...
    rearranged_sequence, BST = build_complete_BST(A)

    # Print the re-arranged data items
    print("\nRe-arranged Data Sequence (from complete_BST_seq):")
    ...
    print("Original Data Sequence:")
    print(mydataSet)
    rearranged_sequence, BST = build_complete_BST(mydataSet)
    print("\nRe-arranged Data Sequence (from complete_BST_seq):")
    print(rearranged_sequence)
    printTree(BST)
